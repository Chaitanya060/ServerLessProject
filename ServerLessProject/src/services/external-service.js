...
export function getCountryListByRegion(region) {
  return _countriesList.filter((country) => country.region === region);
}

export function getCountryListByLanguage(language) {
  return _countriesList.filter((country) =>
    Object.values(country.languages).includes(language)
  );
}

export function getRegionsList() {
  return Object.values(_regionsList);
}
